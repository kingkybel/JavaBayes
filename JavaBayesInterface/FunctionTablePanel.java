...
public final class FunctionTablePanel extends Panel
{
    ...
    public void insert_table(double new_table[])
    {
        ...
        else if (all_variable_names.length == 2)
        {
            ...
            {
                for (j = 0; j < all_variable_values[1].length;
                     j++)
                {
                    fields[i][j].setText(String.valueOf(table_values[i *
                                                                     all_variable_values[1].length +
                                                                     j]));
                }
            }
        }
            ...
            for (i = 0; i < all_variable_values[0].length; i++)
            {
                for (j = 0; j < all_variable_values[1].length; j++)
                {
                    ...
                    fields[i][j].setText(String.valueOf(
                            table_values[get_location_from_indexes(
                                    parent_indexes)]));
                }
            }
    ...
    private void build_distribution_panel()
    {
        ...
        else
        {
            ...
            fields =
            new TextField[all_variable_values[0].length][all_variable_values[1].length];
            ...
            first_parent_values_labels =
            new Label[all_variable_values[1].length];
            ...
            for (j = 0; j < all_variable_values[0].length; j++)
            {
                ...
                for (i = 0; i < all_variable_values[1].length; i++)
                {
                    ...
                    fields[j][i].setText(String.valueOf(
                            table_values[(j * all_variable_values[1].length + i) *
                                         jump]));
                    dp.add(fields[j][i]);
                }
            }
            ...
            for (i = 0; i < fields.length; i++)
            {
                for (j = 0; j < fields[i].length; j++)
                {
                    try
                    {
                        ...
                        parent_indexes[0] = i;
                        parent_indexes[1] = j;
                        table_values[get_location_from_indexes(parent_indexes)] =
                        value_set;
                    }
